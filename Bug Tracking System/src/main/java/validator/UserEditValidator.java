...
public class UserEditValidator implements Validator {
	...
	public void validate(Object object, Errors errors) {
		...
		if (isValidLength(user.getLogin(),6,12)) {
		...
		if (!isValidLength(user.getLogin(),6,12)) {
		if (onlyEng.matcher(user.getLogin()).find() && userService.uniqueLogin(user.getLogin()) != null //такой логин есть
				&& userService.getLoginById(user.getUserId()).equals(user.getLogin()) != true)  { // начальный не равен конечному
			System.out.println(userService.uniqueLogin(user.getLogin()) +" " +userService.getLoginById(user.getUserId()) + " and " + user.getLogin());
			errors.rejectValue("login", "label.user.notcreated");			
		}}
		if (isValidLength(user.getPassword(),6,100)) {
			errors.rejectValue("password", "label.login.passwordNotCorrectEdit");
		}
		if (user.getPassword().toString().length()>100){
			errors.rejectValue("password", "label.login.tooBig");
		}	
